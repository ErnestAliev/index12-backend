    ...
    try {
      ...
      const _openAiChatFromSnapshot = async (qText) => {
        ...
        console.log('🔍 AI DATA PACKET DEBUG:', {
          ...
          pastOps: pastOps.length,
          futureOps: futureOps.length,
          incomeOps: (dataPacket?.operations || []).filter(op => op.kind === 'income').length,
          expenseOps: (dataPacket?.operations || []).filter(op => op.kind === 'expense').length,
          sampleOps: (dataPacket?.operations || []).slice(0, 3).map(op => ({
            kind: op.kind,
            date: op.date,
            amount: op.amount,
            source: op.source
        ...
        const system = [
          ...
          '',
          'КРИТИЧНО - ДОХОДЫ/РАСХОДЫ ЗА ПЕРИОД:',
          '- При запросе "покажи доходы" или "покажи расходы" - используй DATA.operations',
          '- Фильтруй по kind (income/expense) и периоду (если указан)',
          '- Если период включает будущее - раздели на ФАКТ (до сегодня) и ПРОГНОЗ (после сегодня)',
          '- Суммируй операции и показывай итог с разбивкой',
          '',
