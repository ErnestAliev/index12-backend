...
async function generateConversationalResponse({
    ...
    const systemPrompt = [
        ...
        'Rule #4 (Расчет Маржинальности):',
        '   - Всегда считай NPM = (Чистая Прибыль / Факт Доходов) * 100.',
        '   - Выводи результат в скобках рядом с суммой прибыли (округляй до целого, например "75%").',
        '   - Если Факт Доходов = 0, указывай Маржинальность: 0%.',
        '',
        'ПРАВИЛА АНАЛИЗА:',
        'Rule "Summation" (Суммирование обязательств):',
        '   - Если на одну дату приходится несколько платежей одной категории (например, "Налоги"), ТЫ ОБЯЗАН ИХ СУММИРОВАТЬ.',
        '   - Запрещено выводить только один транш. Выводи общую сумму обязательств на дату.',
        'Rule "P&L Integrity" (Чистота P&L):',
        '   - Чистая прибыль = (Факт Доходов - Факт Расходов).',
        '   - СТРОГО ЗАПРЕЩЕНО вычитать из Фактической Прибыли будущие Плановые Разрывы. P&L — это отчет о прошлом, не смешивай его с прогнозом.',
        'Проверка формулы P&L перед выводом:',
        '   - Если (Доход - Расход) не равно (Чистая Прибыль в ответе), пересчитай перед отправкой ответа.',
        '',
        'Используй этот шаблон для TYPE A (Общий статус):',
        '',
        '1. Финансовый результат (P&L)',
        'Факт доходов: [Полное число] ₸',
        '> Расходы: [Полное число] ₸ ([Процент расхода]% от выручки)',
        '> Чистая прибыль: [Полное число] ₸ (Маржинальность: [Рассчитать %]%)',
        '> Эффективность: [Если Маржа > 30% -> "Высокая", иначе "Средняя/Низкая"]',
        '',
