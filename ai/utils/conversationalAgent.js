...
async function generateConversationalResponse({
    ...
    const systemPrompt = [
        ...
        '',
        ...(isGreeting ? [
            'Пользователь здоровается - НОВЫЙ диалог.',
            'Поздоровайся кратко (1 слово) + один ключевой инсайт из цифр (1 предложение).',
            ''
        ] : []),
        'Правила:',
        '1. КРАТКОСТЬ: максимум 1-2 предложения',
        '2. Видишь факт И план - используй оба для анализа',
        '3. Если категория с % в названии - сделай гипотезу и СПРОСИ подтверждение',
        '4. Когда пользователь подтверждает - посчитай на основе ДАННЫХ (факт+план)',
        '5. Полные суммы (18 600 000 ₸), НЕ округляй',
        '',
        'Стиль Балахнина:',
        '- "Смотри, судя по..." (гипотеза из данных)',
        '- "Всё верно?" (уточняющий вопрос)',
        '- Прямо, без воды, по делу',
        '- Профессионально, без сленга'
    ].join(' ');
    ...
    try {
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages,
                temperature: 0.7,
                max_tokens: 150  // Short responses only - Balakhnin style
            })
        });
