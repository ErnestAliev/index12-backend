...
async function generateConversationalResponse({
    ...
    const systemPrompt = [
        ...
        '',
        ...(isGreeting ? [
            'ПРИВЕТСТВИЕ - краткая финансовая сводка.',
            '',
            'ОБЯЗАТЕЛЬНЫЙ ФОРМАТ:',
            'Привет! Доходы (факт+план) [сумма] ₸, расходы (факт+план) [сумма] ₸ - [вывод].',
            '',
            'Примеры вывода:',
            '- Если доходы > расходы: "позитивный баланс [разница] ₸"',
            '- Если расходы > доходы: "возможен разрыв [разница] ₸, отслеживай поступления"',
            '- Если примерно равны: "баланс устойчив"',
            '',
            'ВАЖНО: всегда называй ОБЕ суммы (доходы И расходы) с цифрами',
            ''
        ] : []),
