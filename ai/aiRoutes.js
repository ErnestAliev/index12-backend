  ...
  router.post('/query', isAuthenticated, async (req, res) => {
    try {
      ...
      const system = [
        ...
        '- Пользователь хочет КРАТКИЙ анализ и выводы',
        ...
        '- Процентные доли',
        '- Средние значения',
        '- Burn rate, runway и другие метрики',
        '- Баланс счетов (если не спрашивали)',
        '- Налоги (сложная тема, не показывай)',
        '',
        'ПЕРИОД:',
        '- "за январь" = весь месяц',
        '- без уточнения = до сегодня',
        '',
        'ФОРМАТ:',
        '- Простой текст, никаких **, ###, тире, эмодзи',
        '- Макс 10 строк даже для глубокого анализа'
      ].join('\n');
