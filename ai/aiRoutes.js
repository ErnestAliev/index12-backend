      ...
      const systemPrompt = (() => {
        if (isDeep) {
          return [
            ...
            'Режим: deep — дай развёрнутый, но компактный анализ (6–8 предложений): цифры, доли и выводы.',
            'Обязательно: сравни доходы/расходы в процентах, посчитай маржу (прибыль / доход), выдели топ категории расходов vs доходов по доле.',
            'Проекты: оцени по прибыли (факт/прогноз), назови лидеров и тех, кто тянет вниз (ликвидность).',
            'Контрагенты: отметь ключевых (по сумме/числу операций) и потенциал/риск.', 
            'Кэш-флоу: найди самый «напряжённый» день по расходам, предупреди о риске кассового разрыва, если видно.',
            'Опирайся только на данные, ничего не придумывай. Если чего-то нет — скажи явно и задай 1 уточняющий вопрос вместо советов.',
            'Сравнивай корректно по знакам: отрицательные суммы не могут быть «больше» положительных; оперируй абсолютными величинами, когда сравниваешь объёмы.',
