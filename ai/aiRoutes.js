  ...
  router.post('/query', isAuthenticated, async (req, res) => {
    try {
      ...
      const system = [
        ...
        '',
        'КРИТИЧЕСКИ ВАЖНО:',
        '- Список операций только для ТВОИХ расчетов',
        '- НЕ ПОКАЗЫВАЙ пользователю: операции, ТОП-3, ТОП-5, проценты, детали',
        '- Пользователь хочет ТОЛЬКО краткие выводы',
        '- ДАЖЕ при запросе "глубокий анализ" - будь краток',
        '',
        'ПРИМЕР ПРАВИЛЬНОГО ОТВЕТА:',
        'Запрос: "глубокий анализ"',
        'Ответ:',
        'Доходы: 22 519 166 ₸',
        'Расходы: 4 232 348 ₸',
        'Чистая прибыль: 18 286 818 ₸',
        '',
        'Наблюдения:',
        'Акмекен дает 70% доходов - риск концентрации',
        'Коммуналка 30% расходов - можно оптимизировать',
        '',
        'Рекомендации:',
        'Найти 2-3 дополнительных источника дохода',
        'Провести аудит коммунальных услуг',
        '',
        'ЗАПРЕЩЕНО показывать:',
        '- Нумерованные разделы (1) 2) 3)',
        '- ТОП списки',
        '- Процентные доли в скобках',
        '- Burn rate, runway',
        '- Средние значения',
        '- Налоги',
        '',
        'ФОРМАТ:',
        '- Простой текст без **, ### и цифр в скобках',
        '- Максимум 10 строк'
      ].join('\n');
