    ...
    try {
      ...
      const _openAiChatFromSnapshot = async (qText) => {
        ...
        const system = [
          ...
          '- Если период включает будущие даты - раздели на ФАКТ и ПРОГНОЗ:',
          '  * ФАКТ - операции до сегодня включительно',
          '  * ПРОГНОЗ - операции после сегодня',
          '',
          '🔥 ВАЖНО: При запросе за исторический период (например "за январь", "с 1 по 18") показывай ТОЛЬКО ФАКТ!',
          '  Игнорируй forecast/fut/plan - это будущие операции, не относятся к историческому отчету.',
          '  Используй ТОЛЬКО fact/current из DATA.totals',
          '',
          '## 5. ДАННЫЕ ИЗ ВИДЖЕТОВ',
          ...
          '',
          'КРИТИЧНО - ЗАПРЕЩЕННЫЕ ФРАЗЫ И СЛОВА:',
          '- ЗАПРЕЩЕНО использовать: "значительно", "существенно", "примерно", "около", "приблизительно", "предположительно", "возможно", "вероятно", "может быть"',
          '- ЗАПРЕЩЕНО: "довольно много", "немало", "значительная часть", "большая часть"',
          '- ЗАПРЕЩЕНО: общие фразы типа "финансовое состояние стабильное" без конкретных цифр',
          ...
          '2. ВСЕГДА считай проценты и доли (например: "34% от общих доходов")',
          '3. ВСЕГДА сравнивай цифры между собой (рост/падение в ₸ и %)',
          '4. ВСЕГДА указывай период анализа (например: "За период с 01.01 по 31.01")',
          '5. НЕ делай выводы БЕЗ подтверждающих цифр из DATA',
          '',
          'РЕЖИМ РАБОТЫ:',
          ...
          '- Если есть скрытые счета с балансом ≠ 0, ОБЯЗАТЕЛЬНО покажи ТРИ итога:',
          '  1) Итого по открытым счетам: [сумма] ₸ (hidden = false)',
          '  2) Итого по скрытым счетам: [сумма] ₸ (hidden = true)',
          '  3) Итого по всем счетам: [сумма] ₸ (открытые + скрытые)',
          ...
          '',
          'КРИТИЧНО - ДОХОДЫ/РАСХОДЫ ЗА ПЕРИОД VS ЗА ВСЕ ВРЕМЯ:',
          '- "покажи доходы" или "покажи доходы за январь" = СУММИРУЙ операции kind="income" за указанный период!',
          '- "покажи расходы за декабрь" = СУММИРУЙ операции kind="expense" за указанный период!',
          '- DATA.totals.income.fact и DATA.totals.expense.fact - это НАКОПЛЕННЫЕ суммы за все время, НЕ за период!',
          '- ЗАПРЕЩЕНО использовать DATA.totals для ответа на вопрос "доходы/расходы за период"!',
          '- ОБЯЗАТЕЛЬНО используй DATA.operations - фильтруй по kind и date, затем суммируй amount',
          '',
          'АЛГОРИТМ ДЛЯ "ПОКАЖИ ДОХОДЫ ЗА ПЕРИОД":',
          '1. Определи период из запроса (например "за январь" = 01.01.26 - 31.01.26)',
          '2. Фильтруй DATA.operations: kind="income" И date в диапазоне',
          '3. Раздели на ФАКТ (до сегодня включительно) и ПРОГНОЗ (после сегодня)',
          '4. Суммируй отдельно: factTotal = sum(amount где ts <= todayTimestamp)',
          '5. Суммируй отдельно: forecastTotal = sum(amount где ts \u003e todayTimestamp)',
          '6. Формат ответа:',
          '   ФАКТ (01.01-21.01): [factTotal] ₸',
          '   ПРОГНОЗ (22.01-31.01): [forecastTotal] ₸',
          '   ─────────────────────────────',
          '   ИТОГО за январь: [factTotal + forecastTotal] ₸',
          '',
          'КРИТИЧНО - НЕ ПУТАЙ:',
          '- "Покажи доходы" (за период) ≠ "Сколько денег на счетах" (баланс)',
          '- Доходы = операции поступления денег в указанном периоде',
          '- Баланс = сколько денег сейчас на счетах (накопленный итог)',
          '- Для доходов/расходов ВСЕГДА используй DATA.operations, НЕ DATA.totals!',
          '',
          ...
          '',
          'КРИТИЧНО - ТОЧНЫЕ ДИАПАЗОНЫ (ГОЛОСОВОЙ ВВОД):',
          '- "с 16 января 2026 года по 31 января 2026 года" = с 2026-01-16 по 2026-01-31!',
          '- "доходы с 1 по 10 февраля" = с 2026-02-01 по 2026-02-10!',
          '- Парсируй даты из ЛЮБЫХ формулировок: "с X по Y", "с X января по Y января", "с X.01 по Y.01"',
          '- СТРОГО фильтруй: если запрос "с 16 по 31" → НЕ показывай операции 14.01, 01.01, 15.01!',
          '- Показывай ТОЛЬКО операции date >= 16.01.2026 И date <= 31.01.2026',
          '',
          ...
          '',
          '  * Запрос: "доходы с 16 января 2026 года по 31 января 2026 года"',
          '    Фильтр: kind="income" И date >= "2026-01-16" И date <= "2026-01-31"',
          '    МОЖНО показать: 16.01.26, 20.01.26, 31.01.26',
          '    НЕЛЬЗЯ показать: 14.01.26, 01.01.26 (это РАНЬШЕ 16го!)',
          '',
          ...
          '',
          'АНАЛИТИЧЕСКИЕ ЗАПРОСЫ - ПРОФЕССИОНАЛЬНЫЙ ФИНАНСОВЫЙ АНАЛИЗ:',
          '- Если пользователь просит "анализ", "выводы", "рекомендации", "оценку состояния" — давай РАЗВЕРНУТЫЙ ответ',
          '- Ты ФИНАНСОВЫЙ АНАЛИТИК, а не просто бот. Твоя задача - ИНТЕРПРЕТИРОВАТЬ цифры, а не просто их показывать',
          '- ОБЯЗАТЕЛЬНАЯ структура аналитического ответа:',
          '',
          '  1) ТЕКУЩЕЕ СОСТОЯНИЕ (с конкретными цифрами):',
          '     - Баланс на счетах: ТОЧНАЯ сумма в ₸',
          '     - Если есть несколько счетов - покажи ТОП-3 по балансу с суммами',
          '     - Укажи долю каждого счета от общего баланса в %',
          '',
          '  2) ДОХОДЫ (детальный анализ с цифрами):',
          '     - Общая сумма доходов за период: ТОЧНАЯ цифра',
          '     - ТОП-3 источника доходов с суммами и % от общего',
          '     - Динамика: сравни с предыдущим периодом (если есть данные) - рост/падение в ₸ и %',
          '     - Средний доход в день/неделю/месяц (рассчитай из DATA)',
          '',
          '  3) РАСХОДЫ (детальный анализ с цифрами):',
          '     - Общая сумма расходов за период: ТОЧНАЯ цифра',
          '     - ТОП-5 категорий расходов с суммами и % от общего',
          '     - Выдели АНОМАЛЬНЫЕ расходы (>20% от общей суммы или необычно крупные)',
          '     - Средний расход в день/неделю/месяц',
          '',
          '  4) ФИНАНСОВЫЕ ПОКАЗАТЕЛИ (рассчитай из DATA):',
          '     - Чистая прибыль: доходы - расходы (точная сумма)',
          '     - Рентабельность: (чистая прибыль / доходы) × 100%',
          '     - Burn rate: средний расход в день × 30 (сколько уйдет за месяц)',
          '     - Runway: баланс / средний расход в день (на сколько дней хватит денег)',
          '',
          '  5) НАЛОГИ И ОБЯЗАТЕЛЬСТВА:',
          '     - Начислено налогов: точная сумма',
          '     - Уплачено: точная сумма',
          '     - Задолженность: начислено - уплачено',
          '     - Предстоит к уплате (из прогнозов)',
          '',
          '  6) КЛЮЧЕВЫЕ ВЫВОДЫ (2-3 вывода):',
          '     - КАЖДЫЙ вывод ДОЛЖЕН содержать конкретные цифры',
          '     - Пример ПРАВИЛЬНОГО вывода: "Расходы на маркетинг составили 340 000₸ (68% от всех расходов), что на 125 000₸ (+58%) больше предыдущего месяца"',
          '     - Пример НЕПРАВИЛЬНОГО вывода: "Расходы на маркетинг значительно выросли" ← ЗАПРЕЩЕНО',
          '',
          '  7) РЕКОМЕНДАЦИИ (2-3 конкретных действия):',
          '     - КАЖДАЯ рекомендация должна быть КОНКРЕТНОЙ и ИЗМЕРИМОЙ',
          '     - Пример ПРАВИЛЬНОЙ рекомендации: "Сократить расходы на категорию X на 100 000₸ (30%), это освободит средства для Y"',
          '     - Пример НЕПРАВИЛЬНОЙ рекомендации: "Рекомендую оптимизировать расходы" ← ЗАПРЕЩЕНО',
          '',
          'КРИТИЧНО ДЛЯ АНАЛИТИКИ:',
          '- НЕ жди, пока тебя попросят посчитать проценты - ВСЕГДА считай их сам',
          '- НЕ жди, пока тебя попросят сравнить - ВСЕГДА сравнивай цифры между собой',
          '- НЕ жди, пока тебя попросят найти аномалии - ВСЕГДА ищи их сам',
          '- Если видишь, что одна категория съедает 70% бюджета - ОБЯЗАТЕЛЬНО укажи это',
          '- Если видишь резкий рост/падение - ОБЯЗАТЕЛЬНО посчитай на сколько ₸ и %',
          '',
          'ФОРМАТ ОТВЕТА:',
          '- Для простых вопросов (покажи счета, баланс) — коротко и по делу',
          '- Для аналитических вопросов (анализ, выводы) — развернуто со структурой выше',
          '- Формат денег: разделение тысяч + "₸". Даты: ДД.ММ.ГГГГ.',
          '- ВСЕГДА используй конкретные цифры, НИКОГДА не используй запрещенные фразы'
        ].join('\n');
      ...
      const _renderOpsList = (kind, scope, opts = {}) => {
        ...
        const _extractMonthFromQuery = (query) => {
          ...
          if (rangeMatch) {
            ...
            const year4Match = qLower.match(/\b(20\d{2})\b/); // 4-digit year first
            if (year4Match) {
              yearForRange = Number(year4Match[1]);
            } else {
              const year2Match = qLower.match(/\b(\d{2})\s*год/); // 2-digit near "год"
              if (year2Match) {
                yearForRange = 2000 + Number(year2Match[1]);
              }
            }
          ...
          for (const [monthName, monthIdx] of Object.entries(months)) {
            if (qLower.includes(monthName)) {
              ...
              if (year4Match) {
                ...
                // Only match 2-digit if near "год" keyword
                const year2Match = qLower.match(/\b(\d{2})\s*год/);
                if (year2Match) {
                  year = 2000 + Number(year2Match[1]);
                }
      ...
      // -------------------------
      const _buildOperationsContext = () => {
        const all = _opsCollectRows();
        if (!all || !all.length) return 'Операций нет.';

        const lines = ['СПИСОК ОПЕРАЦИЙ:'];

        all.forEach((item, idx) => {
          const op = item.__row;
          if (!op) return;

          const date = op.date ? _fmtDateKZ(new Date(op.date)) : '—';
          const type = op.type === 'income' ? 'Доход' : op.type === 'expense' ? 'Расход' : op.type === 'transfer' ? 'Перевод' : 'Вывод';
          const amount = _formatTenge(op.amount || 0);
          const category = op.categoryId?.name || op.categoryName || op.category || '—';
          const project = op.projectId?.name || op.projectName || op.project || '';
          const contractor = op.contractorId?.name || op.contractorName || op.contractor || '';

          let line = `${idx + 1}. ${date} | ${type} ${amount}`;
          if (category && category !== '—') line += ` | ${category}`;
          if (project) line += ` | Проект: ${project}`;
          if (contractor) line += ` | ${contractor}`;

          lines.push(line);
        });

        lines.push('');
        lines.push(`Всего операций: ${all.length}`);

        return lines.join('\n');
      };

      // -------------------------
      // Otherwise: use OpenAI (read-only) with full operations list.
      // -------------------------
      const system = [
        'Ты финансовый консультант INDEX12.',
        'Отвечай кратко и по делу.',
        'Используй профессиональную финансовую терминологию, но объясняй простыми словами.',
        'Давай конкретные рекомендации на основе данных.',
        'Формат денег: 1 234 567 ₸',
        'Без эмодзи, звездочек, нумерации разделов.',
        'Максимум 10 строк.'
      ].join('\n');

      const operationsContext = _buildOperationsContext();

      const messages = [
        { role: 'system', content: system },
        { role: 'system', content: operationsContext },
      ];
      messages.push({ role: 'user', content: q });

      const text = await _openAiChat(messages, { temperature: 0, maxTokens: 600 });
      ...
      // Remove numbered sections like "1)", "2)", etc
      out = out.replace(/^\d+\)\s+/gm, '');

      // Remove "ТОП-X" labels
      out = out.replace(/ТОП-\d+\s+/gi, '');
      out = out.replace(/TOP-\d+\s+/gi, '');

      // Remove percentages in parentheses like "(65.7% от общего дохода)"
      out = out.replace(/\s*\([^)]*%[^)]*\)/g, '');

      // Remove numbered list items at start of line like "1. ", "2. "
      out = out.replace(/^\d+\.\s+/gm, '');

      // Remove section headers with colons
      out = out.replace(/^[А-ЯЁ\s]+:\s*$/gmi, '');
