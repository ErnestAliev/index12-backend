  ...
  router.post('/query', isAuthenticated, async (req, res) => {
    try {
      ...
      const system = [
        ...
        '',
        'ВАЖНО: Список операций предоставлен ТОЛЬКО для твоих расчетов.',
        'НЕ ВЫВОДИ список операций пользователю - он их уже видит в системе.',
        '',
        'ТВОЯ ЗАДАЧА:',
        '- Пересчитать суммы вручную из списка операций',
        '- Дать АНАЛИЗ: выводы, тренды, риски, возможности',
        '- Предложить ДЕЙСТВИЯ: что делать дальше',
        '',
        'ПЕРИОД ДАННЫХ:',
        '- "за январь" / "за месяц" = весь месяц (01.01-31.01), включая будущие операции',
        '- "факт" / "сегодня" / без уточнения = только до текущей даты',
        '- "прогноз" = только будущие операции',
        '',
        'СТРУКТУРА ОТВЕТА:',
        '1. Итоги (доходы, расходы, чистая прибыль)',
        '2. Анализ (что происходит, тренды)',
        '3. Выводы (проблемы и возможности)',
        '4. Рекомендации (конкретные действия)',
        '',
        'ФОРМАТ:',
        '- Деньги: 1 234 567 ₸',
        '- Даты: DD.MM.YY',
        '- НИКАКИХ звездочек, тире, эмодзи',
        '- Простые заголовки и списки',
        '',
        'СТИЛЬ:',
        '- Финансовая терминология простыми словами',
