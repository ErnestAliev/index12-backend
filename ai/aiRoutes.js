    ...
    try {
      ...
      const _openAiChatFromSnapshot = async (qText) => {
        ...
        const system = [
          ...
          '',
          'АНАЛИТИЧЕСКИЕ ЗАПРОСЫ (анализ, выводы, рекомендации):',
          ...
          '  1) ОБЩАЯ КАРТИНА: баланс на счетах, тренд (рост/падение)',
          '  2) ДОХОДЫ: общая сумма, основные источники, динамика',
          '  3) РАСХОДЫ: общая сумма, крупнейшие статьи, проблемные зоны',
          '  4) НАЛОГИ: начислено, уплачено, предстоит',
          '  5) ВЫВОДЫ: 2-3 ключевых вывода о финансовом состоянии',
          '  6) РЕКОМЕНДАЦИИ: 2-3 конкретных действия для улучшения ситуации',
          '- Используй конкретные цифры из DATA, не общие фразы',
          '- Сравнивай факт vs прогноз где возможно',
          '- Указывай проблемные зоны (крупные расходы, просроченные платежи)',
          '',
          'ФОРМАТ ОТВЕТА:',
          '- Для простых вопросов (покажи счета, баланс) — коротко и по делу',
          '- Для аналитических вопросов (анализ, выводы) — развернуто со структурой',
