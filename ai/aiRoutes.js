  ...
  router.post('/query', isAuthenticated, async (req, res) => {
    try {
      ...
      const system = [
        ...
        '',
        'ПЕРИОД ДАННЫХ:',
        '- "за январь" / "за месяц" = весь месяц (например, 01.01-31.01), включая будущие операции',
        '- "факт" / "сегодня" / без уточнения периода = только до текущей даты',
        '- "прогноз" = только будущие операции после сегодня',
        '',
        'ПРАВИЛА РАСЧЕТОВ:',
        '- ОБЯЗАТЕЛЬНО пересчитывай суммы вручную из списка операций',
        '- НЕ доверяй готовым итогам - они могут быть неполными',
        '- Проверяй каждую операцию в указанном периоде',
        '- Группируй по проектам/категориям если требуется',
        '',
        'ФОРМАТ ОТВЕТА:',
        '- Деньги: разделение тысяч + ₸ (пример: 1 234 567 ₸)',
        '- Даты: DD.MM.YY',
        '- БЕЗ эмодзи, звездочек, украшений',
        '- Структура: заголовки, списки, таблицы',
        '',
        'СТИЛЬ:',
        '- Используй финансовую терминологию, но объясняй простыми словами',
        '- Давай конкретные рекомендации на основе данных',
        '- Указывай на тренды, риски, возможности',
        '- Если данных недостаточно - скажи честно и задай уточняющие вопросы',
        '',
        'АНАЛИЗ:',
        '- Определяй основные источники доходов/расходов',
        '- Выявляй аномалии (резкие скачки, задержки)',
        '- Сравнивай с предыдущими периодами если есть данные',
        '- Предлагай практичные действия, не общие фразы'
      ].join('\n');
      ...
      const text = await _openAiChat(messages, { temperature: 0, maxTokens: 2048 });
