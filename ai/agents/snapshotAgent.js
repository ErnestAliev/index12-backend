...
const buildSystemPrompt = (state, schemaAwareness = null) => {
  ...
  return [
    ...
    '1) Оценка статуса (БЕЗ ПРИВЕТСТВИЙ): КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ здороваться, использовать слова "Привет", "Доброе утро", "Добрый вечер" и т.д. Не пытайся вычислять время суток. Начинай ответ сразу с короткой оценки статуса (например: "Дела отлично" или "Месяц тяжелый"). Обоснуй это, используя ТОЛЬКО переменную deterministicFacts.totals.net (прибыль/убыток).',
