...
// 🟢 ВАЖНО: При локальной разработке FRONTEND_URL должен быть localhost
...
// --- CONFIG ---
app.use(session({
    secret: process.env.GOOGLE_CLIENT_SECRET || 'dev_secret', 
    resave: false,
...
app.get('/api/balances/snapshot', isAuthenticated, async (req, res) => {
    try {
        ...
        const snapshot = {
            accounts: {}, categories: {}, companies: {},
            contractors: {}, projects: {}, individuals: {}, prepayments: {} 
        };
        ...
        if (transfersOut[0]) {
            addToMap('accounts', transfersOut[0].accounts, -1);
...
// --- ОСТАЛЬНЫЕ МАРШРУТЫ (Экспорт, CRUD, и т.д.) ---
app.get('/api/events/all-for-export', isAuthenticated, async (req, res) => {
    try {
        const userId = req.user.id;
        const events = await Event.find({ userId: userId }).sort({ date: 1 }) 
            .populate(['accountId', 'companyId', 'contractorId', 'projectId', 'categoryId', 'prepaymentId', 'individualId', 'fromAccountId', 'toAccountId', 'fromCompanyId', 'toCompanyId', 'fromIndividualId', 'toIndividualId']); 
        res.json(events);
...
app.post('/api/import/operations', isAuthenticated, async (req, res) => {
  ...
  if (!Array.isArray(operations) || operations.length === 0) return res.status(400).json({ message: 'No operations.' });
  ...
  try {
    for (let i = 0; i < rowsToImport.length; i++) {
      ...
      if (opData.type === 'transfer' || !opData.date || !opData.amount || !opData.type) continue;
...
const generateCRUD = (model, path) => {
    ...
    app.post(`/api/${path}`, isAuthenticated, async (req, res) => {
        try {
            ...
            if (model.schema.paths.order) { const max = await model.findOne({ userId }).sort({ order: -1 }); createData.order = max ? max.order + 1 : 0; }
            if (path === 'accounts') { createData.initialBalance = req.body.initialBalance || 0; createData.companyId = req.body.companyId; createData.individualId = req.body.individualId; }
            if (path === 'contractors') { createData.defaultProjectId = req.body.defaultProjectId; createData.defaultCategoryId = req.body.defaultCategoryId; }
...
const generateBatchUpdate = (model, path) => {
  app.put(`/api/${path}/batch-update`, isAuthenticated, async (req, res) => {
    try {
      ...
      await Promise.all(items.map(item => model.findOneAndUpdate({ _id: item._id, userId }, item)));
...
const generateDeleteWithCascade = (model, path, foreignKeyField) => {
  app.delete(`/api/${path}/:id`, isAuthenticated, async (req, res) => {
    try {
      ...
      if (!deletedEntity) return res.status(404).json({ message: 'Not found' });
...
generateCRUD(Account, 'accounts'); generateCRUD(Company, 'companies'); generateCRUD(Individual, 'individuals'); generateCRUD(Contractor, 'contractors'); generateCRUD(Project, 'projects'); generateCRUD(Category, 'categories'); generateCRUD(Prepayment, 'prepayments');
generateBatchUpdate(Account, 'accounts'); generateBatchUpdate(Company, 'companies'); generateBatchUpdate(Individual, 'individuals'); generateBatchUpdate(Contractor, 'contractors'); generateBatchUpdate(Project, 'projects'); generateBatchUpdate(Category, 'categories');
generateDeleteWithCascade(Account, 'accounts', 'accountId'); generateDeleteWithCascade(Company, 'companies', 'companyId'); generateDeleteWithCascade(Individual, 'individuals', 'individualId'); generateDeleteWithCascade(Contractor, 'contractors', 'contractorId'); generateDeleteWithCascade(Project, 'projects', 'projectId'); generateDeleteWithCascade(Category, 'categories', 'categoryId');

if (!DB_URL) { console.error('Ошибка: DB_URL не установлена!'); process.exit(1); }
mongoose.connect(DB_URL).then(() => { console.log('MongoDB connected.'); app.listen(PORT, () => { console.log(`Server running on port ${PORT}`); }); }).catch(err => { console.error('MongoDB connection error:', err); });
